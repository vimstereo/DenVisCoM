...
class MambaVisionMixer(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        z_l = F.silu(F.conv1d(input=z_l, weight=self.conv1d_z1.weight, bias=self.conv1d_z1.bias, padding='same', groups=self.d_inner//2))
        z_r = F.silu(F.conv1d(input=z_r, weight=self.conv1d_z2.weight, bias=self.conv1d_z2.bias, padding='same', groups=self.d_inner//2))
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        # print('\n in attention')
        B, N, C = x.shape
        # print('x sa both shape',x.shape)
        #print(self.attn)
        if self.attn == 'sa' or self.attn == 'both':
            ...
            #print('sa donoe')
            if self.attn != 'both':
                # print('x afte attn sa/both',x.shape)
                return x
        if self.attn == 'ca' or self.attn == 'both':
            # print('ca/both x shape',x.shape)
            x1, x2 = x.chunk(2, dim=0)
            #print('ca')
            def process_attention(q, kv):
                qkv = self.qkv(torch.cat([q, kv], dim=0)).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
                q, k, v = qkv.unbind(0)
                q, k = self.q_norm(q[:B//2]), self.k_norm(k[B//2:])
                v = v[B//2:]
                ...
                return x.transpose(1, 2).reshape(B//2, N, C)  
            #q and kv are args of the function
            # for process_attention(q,kv) kv gets weighted and returned        
            out1 = process_attention(x2, x1)
            out2 = process_attention(x1, x2)

            x = torch.cat([out1, out2], dim=0)

            x = self.proj(x)
            x = self.proj_drop(x)
            # print('after attn ca/both',x.shape)
            return x
...
class Block(nn.Module):
    def __init__(self,
        ...
        if counter in transformer_blocks:
            # print('counter: ',counter,'trans block: ',transformer_blocks)
            self.bloc = 'attention'
            # print('in attention')
            ...
            self.bloc='mamba'
            # print('in mamba')
            # self.mixer= Mamba2Simple(d_model=dim,d_state=8,d_conv=3,expand=1)
    ...
    def forward(self, x):
        # print(self.bloc,'before going to mixer x', x.shape)
        x = x + self.drop_path(self.gamma_1 * self.mixer(self.norm1(x)))
        # print('after mixer x',x.shape)
...
class MambaVisionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if conv:
            ...
            self.blocks = nn.ModuleList([Block(attn,
                                               dim=128,
                                               counter=i, 
    ...
    def forward(self, x):
        # print('before pad shape',x.shape)
        _, _, H, W = x.shape
        #print("INIT", x.shape)
        #input()
        ...
        for _, blk in enumerate(self.blocks):
            ...
            #input()
        if self.transformer_block:
            # print('before window revers',x.shape)
            x = window_reverse(x, self.window_size, Hp, Wp)
            if pad_r > 0 or pad_b > 0:
                x = x[:, :, :H, :W].contiguous()
        #if self.downsample is None:
        #print("FINAL", x.shape)
        # print('after windows revers',x.shape)
        return x
...
class MambaVision(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(depths)):
            ...
            if interweaving == 'cont':
                transformer_blocks=list(range(depths[i]//2+1, depths[i])) if depths[i]%2!=0 else list(range(depths[i]//2, depths[i]))
            elif interweaving == 'inter':
                # print('conditionals',i)
                transformer_blocks=list(range(1,depths[i],2))
                # print('main trans',transformer_blocks)
            else:
                transformer_blocks=[1000]
            conv = False
            level = MambaVisionLayer(attn,
                                     dim=int(dim * 2 ** i),
                                     depth=depths[i],
...
@register_pip_model
@register_model
def mamba_vision_T(attn,interweaving,depth,heads,pretrained=False, **kwargs):
    model_path = kwargs.pop("model_path", "/tmp/mamba_vision_T.pth.tar")
    #depths = kwargs.pop("depths", [0, 0, 4, 2])
    #num_heads = kwargs.pop("num_heads", [0, 0, 8, 16])
    window_size = kwargs.pop("window_size", [0, 0, 14, 7])
