...
from DenVisCor.DenVisCor import DenVisCor
...
def get_args_parser():
    ...
    #model arch
    parser.add_argument('--attn', default='sa', choices=['sa', 'ca','both'])
    parser.add_argument('--interweaving', default='cont', choices=['cont','inter','none'])
    parser.add_argument('--depth', default='0,0,4,8')
    parser.add_argument('--heads', default='0,0,8,16')
...
def main(args):
    ...
    model = UniMatch(feature_channels=args.feature_channels,
                     ...
                     task=args.task,
                     attn=args.attn,
                     interweaving=args.interweaving,
                     depth=list(int(i) for i in args.depth.split(',')),
                     heads=list(int(i) for i in args.heads.split(','))).to(device)
    ...
    if args.eval:
        ...
        if 'sintel' in args.val_dataset:
            results_dict = validate_sintel(model_without_ddp,
                                           ...
                                           evaluate_matched_unmatched=True,
                                           attn_type=args.attn_type,
                                           attn_splits_list=args.attn_splits_list,
                                           corr_radius_list=args.corr_radius_list,
                                           prop_radius_list=args.prop_radius_list,
        ...
        if True:
            ...
            val_file = os.path.join(args.checkpoint_dir, f'{args.depth}_{args.heads}_{args.attn}_{args.interweaving}_results.txt')
            with open(val_file, 'a') as f:
                ...
                metrics = ['chairs_epe', 'chairs_s0_10', 'chairs_s10_40', 'chairs_s40+',
                           ...
                           'kitti_epe', 'kitti_f1', 'kitti_s0_10', 'kitti_s10_40', 'kitti_s40+','sintel_clean_matched','sintel_clean_unmatched','sintel_final_matched','sintel_final_unmatched'
                           ]
    ...
    def custom_collate(batch):
        img1s, img2s, flows, valids = zip(*batch)

        max_h = max(img.shape[-2] for img in img1s)
        max_w = max(img.shape[-1] for img in img1s)

        img1s_padded = [F.pad(img, (0, max_w - img.shape[-1], 0, max_h - img.shape[-2])) for img in img1s]
        img2s_padded = [F.pad(img, (0, max_w - img.shape[-1], 0, max_h - img.shape[-2])) for img in img2s]
        flows_padded = [F.pad(flow, (0, max_w - flow.shape[-1], 0, max_h - flow.shape[-2])) for flow in flows]
        valids_padded = [F.pad(valid, (0, max_w - valid.shape[-1], 0, max_h - valid.shape[-2])) for valid in valids]

        return torch.stack(img1s_padded), torch.stack(img2s_padded), torch.stack(flows_padded), torch.stack(valids_padded)
